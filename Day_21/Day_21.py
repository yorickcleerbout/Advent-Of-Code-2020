with open("Day_21/input.txt") as fp:
...
safe_ingredient = safe_ingredient.difference(
    set(k for value in food_dict.values() for k in value))
safe_count = sum(
    [ingredient in safe_ingredient for ingredient in all_ingredients])
print(
    f"How many times do any of those ingredients appear? (Part 1): {safe_count}")
...
names.sort()
print("What is your canonical dangerous ingredient list? (Part 2): ",
      ",".join([",".join(food_dict[i]) for i in names]))
