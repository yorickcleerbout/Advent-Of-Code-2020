...
cards](https://adventofcode.com/2019/day/22)! You'd like to play a game
of _Combat_, and there's even an opponent available: a small crab that
...
_rounds_: both players draw their top card, and the player with the
...
Once the game ends, you can calculate the winning player's _score_. The
...
small crab to a game of _Recursive Combat_.
...
Then, the game consists of a series of _rounds_ with a few changes:
...
- Before either player deals a card, if there was a previous round in
  ...
  same players' decks, the _game_ instantly ends in a win for player
...
card might be _the lower-valued of the two cards_ if they won the round
...
by making a _copy_ of the next cards in their deck (the quantity of
...
deck in the sub-game would be _copies_ of the next three cards in their
deck.)
...
In the above game, the winning player's score is _`291`_.
