...
with open("Day_24/input.txt") as fp:
...
part_one = sum(list(curr_floor.values()))
...
for _ in range(100):
    ...
    for k, v in new_floor.items():
        ...
        neighbors = sum([floor[(k[0] + o[0], k[1] + o[1])]
                         for o in [(0, -1), (1, -1), (1, 0), (0, 1), (-1, 1), (-1, 0)]])
        if v:
...
part_two = sum(list(new_floor.values()))

print("How many tiles are left with the black side up? (Part 1):", part_one)
print("How many tiles will be black after 100 days? (Part 2):", part_two)
